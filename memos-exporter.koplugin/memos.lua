...
-- http func
local function getMemos(api, token, options)
    -- Memos API use page and limit
    -- see https://www.usememos.com/docs/api/rest#list-memos
    local memos = {}
    local has_more = true
    ...
    local tag = options.tag or "" -- filter by tag
    ...
    local api_url = api
    if not api_url:match("/api/v1/memos$") then
        api_url = api_url .. "/api/v1/memos"
    end
    ...
    repeat
        ...
        if tag ~= "" then
            table.insert(query_params, "tag=" .. tag)
        end

        local url = api_url .. "?" .. table.concat(query_params, "&")
        local sink = {}
        ...
        if response.memos then
            for _, memo in ipairs(response.memos) do
                table.insert(memos, memo.content)
            end
            if #response.memos < pageSize then
                has_more = false
            else
                page = page + 1
            end
        ...
        socket.sleep(0.1)
    until not has_more or (options.max_count and #memos >= options.max_count)
...
function MemosExporter:export(t)
    ...
    for _, booknotes in ipairs(t) do
        local options = {
            limit = 50,
            tag = booknotes.title:gsub("%s+", "_"),
            max_count = 500
        }
        -- get memos by booknotes title
        local memos = getMemos(self.settings.api, self.settings.token, options)
        -- create Highlights not exist in remote memos
        local ok = self:createHighlights(booknotes, memos)
        if not ok then return false end
    end
    return true
end

function MemosExporter:createHighlights(booknotes, memos)
    local number = 0
    ...
    local skip_number = number - (success_number + error_number)
    logger.dbg("createHighlights success number: " ..
        success_number .. " createHighlights error number: " ..
        error_number .. " createHighlights skip number: " .. skip_number)
    return true
end
