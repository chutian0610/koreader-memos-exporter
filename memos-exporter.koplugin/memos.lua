...
function MemosExporter:isReadyToExport()
	if self.settings.api and self.settings.token then
		return true
	end
	return false
end
...
function MemosExporter:getMenuTable()
	return {
		...
		sub_item_table = {
			...
			{
				text = _("Set Memos API URL to filter memos"),
				keep_menu_open = true,
				callback = function()
					local auth_dialog
					auth_dialog = InputDialog:new({
						title = _("Set API URL for Memos"),
						input = self.settings.get_api,
						buttons = {
							{
								{
									text = _("Cancel"),
...
function MemosExporter:createHighlights(booknotes)
	...
	local book_title_tag = booknotes.title:gsub("%s+", "_")
	...
	if not existing_memos then
		logger.warn("Error fetching existing highlights", err)
		return false
	end

	local existing_contents = {}
	if existing_memos.memos and #existing_memos.memos > 0 then
		for _, memo in ipairs(existing_memos.memos) do
			existing_contents[memo.content] = true
		end
	end
	...
	for _, chapter in ipairs(booknotes) do
		for _, clipping in ipairs(chapter) do
			...
			if not existing_contents[highlight] then
				local result, err = makeRequest("POST", { content = highlight }, self.settings.api, self.settings.token)
				if not result then
					logger.warn("Error creating highlight", err)
					error_number = error_number + 1
				end
				number = number + 1
			else
				logger.dbg("Skipping existing highlight:", highlight)
			end
		end
	end
